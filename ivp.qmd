...
algorithms for the solutions of IVPs (this chapter) and BVPs (in Chapter -@sec-bvp).
...
points $x_0,\ldots,x_N\in[a,b]$ are referred to as **mesh points**.]{.aside}
...
back to this point in @sec-onestep. We can continue the scheme for $w_3$, $w_4$,
etc.: 
$$
 w_0 := \alpha,
$$ {#eq-eulerdiff0}
...
Eq. @eq-eulerdiffeq is called the **difference equation** of Euler's method.
...
already knows the exact solution]{.aside}
...
starting away from solution curve, we use Taylor's Theorem yet again:
...
Eq. @eq-linapprox a bit more in detail. We suppose that the
...
[@Steward:1991 Ch. 15]. We give a very brief review here, in our
notation.
...
several variables. This is summarized in
Appendix [4](#app:taylorthm){reference-type="ref"
reference="app:taylorthm"}.
...
Eq. @eq-lteeuler above, we define the local truncation
error
...
following function for our one-step method,
$$
...
^[If you hear someone speaking of "the Runge-Kutta method", he is
...
::: algorithm
::: algorithmic
$x := a$, $\mathbf{w}:= \boldsymbol{\alpha}$, $h := h_{max}$ Compute
$\mathbf{k}_1,\ldots, \mathbf{k}_6$ from Butcher tableau with step size
$h$
$R := {h}^{-1}\lVert  \sum_{j=1}^6  (c_j-\tilde c_j) \mathbf{k}_j \rVert$
$x := x+h$, $\mathbf{w}:= \mathbf{w}+ \sum_{j=1}^5  c_j \mathbf{k}_j$
$q :=(T/2R)^{1/4}$ $h := 0.1h$ $h := 4h$ $h := h \cdot q$ **if**
$h \geq h_{max}$ **then** $h:= h_{max}$ **end if** $h := b-x$
**exception** ("minimum step size exceeded")

$\mathbf{w}$
:::
:::

Algorithm [\[alg:rkf\]](#alg:rkf){reference-type="ref"
reference="alg:rkf"} shows the Runge-Kutta-Fehlberg method in
...
$$
Also, the factor $q$ is limited to the interval $[0.1,4]$ (lines
...
::: {#tbl-abmethod}
  Method                Difference equation                                                                                                                  Error order
  --------------------- ----------------------------------------------------------------------------------------------------------------------------------- -------------
  Two-step explicit     
$$
%                                                                                                                                   $O(h^2)$
                        \mathbf{w}_{i+1} = \mathbf{w}_i + \frac{h}{2} (3 \mathbf{f}_i - \mathbf{f}_{i-1})                                                   
                        %
$$
...
$$
%                                                                                                                               $O(h^3)$
                        \mathbf{w}_{i+1} = \mathbf{w}_i + \frac{h}{12} (5 \mathbf{f}_{i+1} + 8 \mathbf{f}_i - \mathbf{f}_{i-1})                         
                        %
$$
...
::: algorithm
::: algorithmic
$h := (b-a)/N$, $x_0 := a$, $\mathbf{w}_0 := \boldsymbol{\alpha}$,
$\mathbf{f}_0 := \mathbf{f}(x_0,\mathbf{w}_0)$. Compute $\mathbf{w}_i$
using Runge-Kutta fourth order $x_i := x_{i-1}+h$;
$\mathbf{f}_i := \mathbf{f}(x_i,\mathbf{w}_i)$ $x := a + ih$
$\mathbf{w}:= \mathbf{w}_3 +
      \frac{h}{24} (55 \mathbf{f}_3 - 59 \mathbf{f}_2 + 37 \mathbf{f}_1 - 9 \mathbf{f}_0)$
$\mathbf{w}:= \mathbf{w}_3 + \frac{h}{24} (9 \mathbf{f}(x,\mathbf{w}) + 19 \mathbf{f}_3 - 5
   \mathbf{f}_2 + \mathbf{f}_1)$ $x_j := x_{j+1}$;
$\mathbf{w}_j := \mathbf{w}_{j+1}$; $\mathbf{f}_j :=
\mathbf{f}_{j+1}$; $x_3 := x$; $\mathbf{w}_3 := \mathbf{w}$;
$\mathbf{f}_3 := \mathbf{f}(x,\mathbf{w})$; $\mathbf{w}$
:::
:::

Algorithm [\[alg:adams4\]](#alg:adams4){reference-type="ref"
reference="alg:adams4"} shows this predictor-corrector method in
...
^[Cf. Maple worksheet used in the lecture, available on Moodle]
...
ODEs which show this behaviour are called *stiff equations*. An exact
...
<figure id="fig:stabilitytaylor">
<embed src="stability_taylor.pdf" />
<figcaption>Region of stability for Taylor methods</figcaption>
</figure>
...
by the example above. For having a stable method for a large set range
...
\end{aligned}
$$ {#eq-trapezde}
 Applying it to the test equation
@eq-testeq, we obtain
$$
...
<figure id="fig:stabilityadams">
<p>AB4: Adams-Bashforth 4-step; AM3/AM4: Adams-Moulton 3-step/4-step;
ABM43: Adams-Bashforth-Moulton predictor-corrector, 4/3-step. Graphics
taken from <span class="citation" data-cites="TTM"></span>.</p>
<img src="stability_multistep_nolegend.jpg" style="width:45.0%" />
<figcaption>Region of stability for Adams-Bashforth and Adams-Moulton
methods</figcaption>
</figure>
...
Eq. @eq-trapezsolve, we may in general find several solutions.
...
Eq. @eq-trapezsolve. We can generalize this to the case of a
*linear* ODE: 
$$
...
^[For more details on Newton's method for solving (single)
    nonlinear equations, see Part A of this module, or [@Burden_Faires
...
::: algorithm
::: algorithmic
$x := a$,  $w := \alpha$,  $h := (b-a)/N$; $c := w + \frac{h}{2}f(x,w)$;
 $k:=1$; $v := \dfrac{ w - c -
h \, f(x+h, w)/2}{ 1 - h\, \partial_y f(x+h, w)/2}$; $w := w - v$;
**if** $|v|<T$ **then** **break** **end if** $k:=k+1$; **exception**
("Too many iterations") $x := x + h$ $w$
:::
:::

Algorithm [\[alg:trapnewton\]](#alg:trapnewton){reference-type="ref"
reference="alg:trapnewton"} combines the thoughts above, implementing
the Implicit Trapezoidal method using Newton's method. It contains two
loops: The outer loop (lines 3--15) is the usual iteration of our
one-step and multi-step methods, counting through the mesh points. In
each iteration of this loop, there is another loop (lines 5--12) which
implements Newton's method. This inner loop does not have a fixed number
of steps a priori. We terminate it either if the desired tolerance is
reached (line 8), or (with an error message) if the number of steps
exceeds a certain maximum. This is a safety measure to avoid an endless
loop in case that the Newton iteration does not converge for any reason.
