...
# A function that takes a vector of strings for the header and a data frame
# and returns a markdown table
function df_to_md(df::DataFrame, header::Vector{String})
    # Get the number of columns
    ncol = size(df, 2)
    # Get the number of rows
    nrow = size(df, 1)
    # Create a vector of strings for the table
    table = Vector{String}(undef, nrow + 2)
    # Add the header
    table[1] = join(header, " | ")
    # Add the separator
    table[2] = join(repeat(["---"], ncol), " | ")
    # Add the data
    for i in 1:nrow
        # Convert the DataFrameRow to a regular Array
        row = collect(df[i, :])
        table[i + 2] = join(row, " | ")
    end
    # Join the table
...
f(x) = cos(x) - x
for i = 1:7
    plot(f, 0, 1, label="cos(x) - x", legend=:topright,
        xlabel="x", ylabel="y", lw=2, legendtitle="Iteration $i")
    hline!([0], color = :black, label="x-axis")
    plot!([a, b], [0, 0], label="interval", lw=10)

    # Find middle point
    x = (a + b) / 2
    scatter!([a], [0], label="start")
    scatter!([b], [0], label="end")
    scatter!([x], [0], label="midpoint")
    frame(p, scatter!([x], [f(x)], label="value"))
...
using DataFrames
N = 10
df = DataFrame(n = Int[], a = Float64[], b = Float64[], x = Float64[], f_x = Float64[])
f(x) = x^2 - 2
a = 1.0
b = 2.0
for i = 1:N
...
df_to_md(df, ["n", "\$a_n\$", "\$b_n\$", "\$x_n\$", "\$f(x_n)\$"])
```
...
diff = abs(df[10, 4] - s2)
Markdown.parse("""
Thus, \$\\sqrt{2}\\approx 1.4150\$ with absolute error smaller than \$10^{-3}\$ (Note that \$\\sqrt{2}=$s2\\dots\$, so that the actual error is \$E=\\vert x_{10}-\\sqrt{2}\\vert=$diff\\ldots\$).
""")
```
...
```{julia}
#| label: fig-false-animation
#| fig-cap: Method of false position
#| code-fold: true
using Plots
p = Animation()
a = 0
...
f(x) = x^2-2
for i = 1:4
    plot(f, 0, 2, label="x^2-2", legend=:topleft,
        xlabel="x", ylabel="y", lw=2, legendtitle="Iteration $i")
    hline!([0], color = :black, label="x-axis")
    plot!([a, b], [f(a), f(b)], label="secant")

    # Find middle point
    x = a - f(a) / (f(b) - f(a)) * (b - a)
    scatter!([a], [f(a)], label="f(a)")
    scatter!([b], [f(b)], label="f(b)")
    scatter!([x], [0], label="x")
    frame(p, scatter!([x], [f(x)], label="f(x)"))
...
using Plots
a = Animation()
# Draw cobweb plot for fixed point iteration for cos(x) = x
using Plots
g(x) = cos(x)
plot(g, 0, π/2, label="g(x)=cos(x)", legend=:topleft)
frame(a, plot!(x->x, label="y=x"))
x = 0.2
y = g(x)
# plot vertical line at x
frame(a, plot!([x, x], [0, y], label="", color=:red))
# plot horizontal line at y
frame(a, plot!([x, y], [y, y], label="", color=:red))
for n in 1:8
    x = y
    y = g(x)
    # plot vertical line at x
    frame(a, plot!([x, x], [x, y], label="", color=:red))
    # plot horizontal line at y
    frame(a, plot!([x, y], [y, y], label="", color=:red))
end
gif(a, fps = 2)
