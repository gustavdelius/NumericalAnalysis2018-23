...
BVP @eq-bvpex1, and by our computation, it is the *only*
solution of the BVP. In short,
@eq-bvpex1 has a unique solution.
...
$$
solves the BVP
@eq-bvpex2. This BVP has *many* solutions.
...
::: {#thm:bvplinunique .theorem}
**Theorem 10**. *Suppose that $p,q,r \in \mathcal{C}^0([a,b])$, and that
...
Theorem [9](#thm:bvpunique){reference-type="ref"
reference="thm:bvpunique"}: In the present case,
$$
...
$$
and continuous
functions on a bounded interval are always bounded. ◻
:::
...
-   the *Shooting Method*, which makes use of the techniques for IVPs
    which we discussed in Part [](#par:ivp){reference-type="ref"
    reference="par:ivp"} of the course,
...
known methods from Part [](#par:ivp){reference-type="ref"
reference="par:ivp"}. We can then compute the correct value of $\gamma$
...
$\gamma$ into @eq-yglin, we have found the (approximate) solution of the
BVP.

::: algorithm
::: algorithmic
boundary values $\alpha,\beta$; number of Runge-Kutta steps $N$
$F :=\; (x,\mathbf{u}) \mapsto (u ^{(2)},p(x) u ^{(2)} + q(x) u ^{(1)} + r(x))$
Rewrite and solve IVP for $y_0$ $\mathbf{y}:=$
$\mathbf{y}= ( \mathbf{y}_0,\ldots, \mathbf{y}_N)$, each $\mathbf{y}_j$
is a 2-vector
$G :=\; (x,\mathbf{u}) \mapsto (u ^{(2)},p(x) u ^{(2)} + q(x) u ^{(1)})$
Rewrite and solve IVP for $z$ $\mathbf{z}:=$
$\mathbf{z}= ( \mathbf{z}_0,\ldots, \mathbf{z}_N)$, each $\mathbf{z}_j$
is a 2-vector $\gamma := \dfrac{\beta - y_N ^{(1)} }{z_N ^{(1)}}$
$w_i := y_i ^{(1)} + \gamma z_i ^{(1)}$ ($w_{0},\ldots,w_{N}$)
:::
:::

Algorithm [\[alg:shootlin\]](#alg:shootlin){reference-type="ref"
reference="alg:shootlin"} summarizes the Linear Shooting Method. We
first rewrite the IVP
@eq-ivpshooty0 for $y_0$ as a system of two first-order
ODEs, and approximate its solution (lines 2--3). For concreteness' sake,
we choose the classical Runge-Kutta method for the approximation. We do
likewise with the IVP
@eq-ivpshootz for $z$, in lines 4--5. Now we can compute the
correct value of $\gamma$ (line 6), and combine the approximations for
$y_0$ and $z$ into the approximation of the BVP solution (lines 7--9).

### Nonlinear Shooting

Now let us turn to the nonlinear case, i.e., to a generic BVP of the
form 
$$
 y''(x) = f(x,y(x),y'(x)),  \quad a \leq x \leq b, \quad y(a) = \alpha, \; y(b) = \beta.
$$ {#eq-bvpshootnonlin}
...
::: algorithm
::: algorithmic
boundary values $\alpha,\beta$; start value $\gamma_0$; number of
Runge-Kutta steps $N$; tolerance $T$; maximum number of Newton
iterations $M$
$F:=  (x,\mathbf{u}) \mapsto \big(u ^{(2)},  f(x,u ^{(1)},u ^{(2)}), u ^{(4)} ,
   \frac{\partial f}{\partial y}(x,u ^{(1)},u ^{(2)}) \, u ^{(3)} +\frac{\partial
    f}{\partial y'}(x,u ^{(1)},u ^{(2)}) \, u ^{(4)} \big)$
$\gamma := \gamma_0$; $k:=0$ []{#ali:nlsnewtonstart
label="ali:nlsnewtonstart"} $\mathbf{w}:=$ here
$\mathbf{w}= ( \mathbf{w}_0,\ldots, \mathbf{w}_N)$, each $\mathbf{w}_j$
is a 4-vector $v := \dfrac{w_{N} ^{(1)}-\beta}{w_{N} ^{(3)}}$;
$\gamma := \gamma - v$ **if** $|v|<T$ **then** **break** **end if**
$k:=k+1$ **if** $k>M$ **then** **exception** ("Too many iterations")
**end if** []{#ali:nlsnewtonend label="ali:nlsnewtonend"}
($w_{0} ^{(1)},\ldots,w_{N} ^{(1)}$)
:::
:::

Let us summarize the Nonlinear Shooting Method in
Algorithm [\[alg:shootnonlin\]](#alg:shootnonlin){reference-type="ref"
reference="alg:shootnonlin"}. Its structure is of the now well-known
Newton iteration type, with the Newton loop in lines
[\[ali:nlsnewtonstart\]](#ali:nlsnewtonstart){reference-type="ref"
reference="ali:nlsnewtonstart"}--[\[ali:nlsnewtonend\]](#ali:nlsnewtonend){reference-type="ref"
reference="ali:nlsnewtonend"}. In each iteration of the loop, we solve
...
cf. Theorem [12](#thm:taylor1d){reference-type="ref"
reference="thm:taylor1d"}: 
$$
\begin{aligned}
{4}
\label{eq:cd1}
...
-   Replace all occurrences of $y'(x_i)$ with
    ...
    cf. [\[eq:cdyp\]](#eq:cdyp){reference-type="eqref"
    reference="eq:cdyp"}.

-   Replace all occurrences of $y''(x_i)$ with
    $\dfrac{w_{i+1}-2 w_i +w_{i-1}}{h^2}$;
    cf. [\[eq:cdypp\]](#eq:cdypp){reference-type="eqref"
    reference="eq:cdypp"}.
...
Part A); or it can be nonlinear, in which case we need Newton's method
in several variables (again see Part A) to approximate the solution.
...
discussed in Part A. To that end, we need to compute the Jacobian matrix
...
step of the Newton iteration very fast.
...
::: algorithm
::: algorithmic
boundary values $\alpha,\beta$; number of mesh points $N$, max. Newton
iterations $M$ $h := (b-a)/(N+1)$; $k:=1$
$w ^{(i)} := \alpha + i h \frac{\beta-\alpha}{b-a}$ $(i = 1,\ldots,N)$
[]{#ali:fdmnewtonstart label="ali:fdmnewtonstart"} Compute
$\mathbf{F}(\mathbf{w})$ and $\mathbf{J}(\mathbf{w})$ Solve
$\mathbf{J}(\mathbf{w})\mathbf{v}=\mathbf{F}(\mathbf{w})$ for
$\mathbf{v}$ using e.g. Crout factorization
$\mathbf{w}:= \mathbf{w}- \mathbf{v}$ **if**
$\lVert \mathbf{v} \rVert<T$ **then** **break** **end if** $k:=k+1$
**if** $k>M$ **then** **exception** ("Too many iterations") **end if**
[]{#ali:fdmnewtonend label="ali:fdmnewtonend"} $\mathbf{w}$
:::
:::

All that's left to do is to assemble the algorithm. This is done in
Algorithm [\[alg:fdmnonlin\]](#alg:fdmnonlin){reference-type="ref"
reference="alg:fdmnonlin"}. The pattern follows our usual approach to
Newton's method.
...
(Burden and Faires Algorithm 6.7) to this end --- this makes use of the
...
::: {#it:miniunique .theorem}
**Theorem 11**. *[]{#thm:rrequiv label="thm:rrequiv"} Let
...
(i) *[]{#it:bvpunique label="it:bvpunique"} $y$ is the unique solution
    of the boundary value problem in
    Eq. [\[e1\]](#e1){reference-type="eqref" reference="e1"}.*

(ii) *$y$ is the unique function in $\mathcal{C}^2_{0}[0,1]$ which
     minimizes the integral $I[y]$ in
     Eq. [\[int\]](#int){reference-type="eqref" reference="int"}.*
:::

::: proof
*Proof.* Here we prove only ([11](#it:miniunique){reference-type="ref"
reference="it:miniunique"}) $\Rightarrow$
([\[it:bvpunique\]](#it:bvpunique){reference-type="ref"
reference="it:bvpunique"}), neglecting the aspect of uniqueness. For a
...
Theorem [\[thm:rrequiv\]](#thm:rrequiv){reference-type="ref"
reference="thm:rrequiv"}. The idea is to approximate the function that
...
[\[trialf\]](#trialf){reference-type="eqref" reference="trialf"} into
$I[\phi]$ in [\[int\]](#int){reference-type="eqref" reference="int"}
gives
$$
...
function $\phi$ from [\[trialf\]](#trialf){reference-type="eqref"
reference="trialf"}.
...
-   They should be simple enough so that the integrals for $A ^{(i,j)}$
    and $b ^{(j)}$ in Eqs. [\[eq:ea\]](#eq:ea){reference-type="eqref"
    reference="eq:ea"} and [\[eq:eb\]](#eq:eb){reference-type="eqref"
    reference="eq:eb"} can reasonably be evaluated - if possible,
    explicitly.
...
$$
An example for a computer
implementation of the algorithm will be discussed in the Practicals.
