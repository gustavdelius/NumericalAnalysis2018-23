...
\Function{NonlinearShooting}{$a,b,\alpha,\beta,\gamma_0,N,T,M$}
  ...
  \State $F:=  (x,\uv) \mapsto \big(u\vc{2},  f(x,u\vc{1},u\vc{2}), u\vc{4} ,
   \frac{\partial f}{\partial y}(x,u\vc{1},u\vc{2}) \, u\vc{3} +\frac{\partial
    f}{\partial y'}(x,u\vc{1},u\vc{2}) \, u\vc{4} \big)$
  \State $\gamma := \gamma_0$;  $k:=0$
  \While{$k\leq M$}
    \State $ \wv := $ 
      \Call{RungeKutta}{$F, a,b, (\alpha,\gamma,0,1), N$}
    \State \Comment{here $\wv = ( \wv_0,\ldots, \wv_N)$, each $\wv_j$ is a 4-vector}
    \State $v := \dfrac{w_{N}\vc{1}-\beta}{w_{N}\vc{3}}$; $\gamma := \gamma - v$
    \If{$|v|<T$}
      \Break
    \EndIf
    \State $k:=k+1$
  \EndWhile
  \Return ($w_{0}\vc{1},\ldots,w_{N}\vc{1}$)
\EndFunction
\end{algorithmic}
\end{algorithm}
```
