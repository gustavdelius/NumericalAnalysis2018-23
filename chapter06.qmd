...
$$ {#eq-6.12}
This property ensures that $P(x)$, given by @eq-interp1, satisfies the required conditions $P(x_0)=f(x_0)$ and $P(x_1)=f(x_1)$. Indeed, 
$$
\begin{split}
...
$$  {#eq-6.16}
This polynomial, called the $n^\text{th}$ *Lagrange interpolating polynomial*, is given by @eq-lagrangeform, where the functions* $L_k(x)$ are given by @eq-c10.
:::

::: {.proof}
It is evident from @eq-c10--@eq-lagrangeform that $\deg P \leq n$. Moreover, by @eq-lkdelta, we have for $k=0,\ldots,n$ 
$$
...
Let us construct the Lagrange interpolating polynomial of degree 2 for this data. From @eq-c10 we have
$$
\begin{split}
...
If $x=x_k$ for some $k$, then $f(x_k)=P(x_k)$ and @eq-c11 holds for any choice of $\xi$. Therefore for the rest of this proof we assume that 
$$ 
...
$$  {eq-6.24}
Since $f\in C^{n+1}[a, b]$, $P\in C^{\infty}[a, b]$ and in view of @eq-approx:2, it follows that $g\in C^{n+1}[a, b]$. Applying $g$ at $t=x$ and at $t=x_k$ for $k=0,\ldots,n$, we obtain 
$$
\begin{split}
...
$$ {#eq-6.27}
Substituting this into @eq-c17, we obtain the equation 
$$ 
0=f^{(n+1)}(\xi)-(f(x)-P(x))\frac{(n+1)! }{ \prod_{i=0}^{n}(x-x_i)}, 
$$ {#eq-6.28}
which is equivalent to @eq-c17. ◻
:::
...
Now let us evaluate the error involved in approximating $f$ by $P$ on the whole interval $[2, 4]$. Again using @eq-approx:3, we have 
$$
\begin{split}
...
$$ {#eq-6.35}
The function $\phi$ has a maximum at $x':=\tfrac{17}{6}-\tfrac{\sqrt{13}}{6}\approx 2.232$, with $\phi(x')\approx0.110$, and a minimum at $x'':=\tfrac{17}{6}+\tfrac{\sqrt{13}}{6}\approx 3.434$, where $\phi(x'')\approx-0.758$. By @eq-approx:4 we obtain 
$$
...
As we saw in @thm-lagrangepoly, the interpolating polynomial (of minimal degree) for a function at distinct points $x_0, x_1, \ldots, x_n$ is unique. However, it can be rewritten in many different ways. The Lagrange form @eq-lagrangeform may not always be the optimal one for numerical purposes, since computing its value requires a large number of multiplications. Here we present an alternative form of the same polynomial, known as the *Newton form*.
...
We now claim that all interpolating polynomials can be written in terms of divided differences, in generalization of equations @eq-dd1 and @eq-dd2.
...
This relation is known as *Newton's divided-difference formula*. Note that an empty product is defined to be equal to $1$, so in the $k=0$ term in @eq-newtonddf the factor $\prod_{0 \leq j < 0} (x-x_0)=1$ , so the first term in the sum is $f[x_0]$.

::: {.proof}
We use induction on $n$. For $n=0$, we have $P(x)=f(x_0)=f[x_0]$ and @eq-newtonddf holds. Now suppose that @eq-newtonddf is already known for $n-1$ in place of $n$. Denote by $\hat P$ the interpolating polynomial through $x_0,\ldots,x_{n-1}$, and with $\check P$ the interpolating polynomial through $x_1,\ldots,x_n$. By the induction hypothesis, 
$$
\begin{split}
...
$${#eq-pfrac} 
Indeed, one verifies the equality at $x=x_0$, and $x=x_1,\ldots,x_{n-1}$, and at $x=x_n$ by a short computation in each case. Then @eq-pfrac holds in generality due to the uniqueness of the interpolating polynomial (established in @thm-lagrangepoly).

Now we compute the leading order coefficient $c_n$ of $P$, i.e., the constant $c_n$ such that $P(x)= c_n x^n + \text{lower order terms}$. From @eq-phat, @eq-pfrac one can directly read off that 
$$
...
\end{split}
$$ {#eq-newtonddf2}
by @eq-phat, which completes the proof. ◻
:::
...
-   We can see from @eq-newtonddf that, if the $k^\text{th}$ divided difference is constant, this means that the degree of the interpolating polynomial is $k$ (because higher divided differences are all zero), so that we do not need to use all the data in the table ($k+1$ points will be enough).
...
$$S(x_{i})=f_{i} \text{ for } i=0, 1, \dots, n;$$ {#eq-dc1}


$$S_{i}(x_{i})=S_{i+1}(x_{i}) \text{ for } i=1, 2, \dots, n-1;$$ {#eq-dc2}

$$S^{\prime}_{i}(x_{i})=S^{\prime}_{i+1}(x_{i}) \text{ for } i=1, 2, \dots, n-1;$$ {#eq-dc3}

$$S^{\prime\prime}_{i}(x_{i})= S^{\prime\prime}_{i+1}(x_{i}) \text{ for } i=1, 2, \dots, n-1.$$ {#eq-dc4}

It is not obvious *a priori* that such an interpolant exists, or, if so, that it is unique. We have $4n$ unknown coefficients $a_{i}$, $b_{i}$, $c_{i}$, $d_{i}$ ($i=1, 2, \dots, n$). [Condition @eq-dc1] gives $n+1$ equations. [Conditions @eq-dc2; -@eq-dc3; -@eq-dc4] give $3(n-1)$ equations. Thus, we have $n+1+3(n-1)=4n-2$ equations for $4n$ unknowns. So, we need to specify two more conditions to define $S(x)$ uniquely. Common choices are:
...
**Natural cubic spline.** Let $h_{i}=x_{i}-x_{i-1}$. [Condition @eq-dc1] gives 
$$ 
...
for $i= 2, \dots, n-1$. Substituting this into @eq-d6 and using @eq-d4 and @eq-d8 we find that 
$$
\begin{split}
...
\end{split}
$$ {#eq-d11}
for $i= 2, \dots, n-1$. Thus, we have obtained $n-2$ linear equations for $n-1$ unknowns $c_{1}, \dots, c_{n-1}$ (we already know that $c_{n}=0$). One more equation is obtained as follows. First, the condition $S(x_{0})=S_{1}(x_{0})=f_{0}$ and @eq-d9 yield 
$$ 
b_{1}=\frac{f_{1}-f_{0}}{h_{1}}+\frac{2}{3}h_{1}c_{1}.
$${#eq-d12}
On substituting this into @eq-d6 for $i=1$ and using @eq-d10 for $i=2$, we obtain 
$$ 
2(h_{1}+h_{2})c_{1}+h_{2}c_{2}=3 \left(\frac{f_{2}-f_{1}}{h_{2}}- \frac{f_{1}-f_{0}}{h_{1}}\right)=3(h_{1}+h_{2})f(x_{0},x_{1},x_{2}) .
$${#eq-d13}
@eq-d11, @eq-d13 can be written as 
$$ 
\left(
\begin{array}{ccccccc}
...
When $c_{1}, \dots, c_{n}$ are known, coefficients $d_{1}\dots, d_{n}$ and $b_{1}\dots, b_{n}$ can be determined using @eq-d8, @eq-d9, @eq-d10 and @eq-d12.
...
Substituting these into @eq-d8--@eq-d10, we find 
$$
\begin{aligned}
